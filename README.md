...
```
This will return nodes with a `node.kubernetes.io/disk-pressure` taint and filter out nodes with a `!node.kubernetes.io/unschedulable:NoSchedule` taint. In other words, show me the capacity on nodes that have high disk pressure but don't show me the ones that can't schedule workloads. 
```
